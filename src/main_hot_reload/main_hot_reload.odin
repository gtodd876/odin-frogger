...
load_game_api :: proc(api_version: int) -> (api: Game_API, ok: bool) {
	...
	_, is_dynlib_initialize_symbols_ok := dynlib.initialize_symbols(
		&api,
		game_dll_name,
		"game_",
		"lib",
...
main :: proc() {
	...
	for game_api.should_run() {
		...
		if is_build_requested {
			...
			_, stdout, stderr, _ := os2.process_exec(
				process_desc,
		...
		game_dll_modification_time, get_modification_time_error := os.last_write_time_by_name(
			GAME_DLL_PATH,
		)
		...
		game_dll_modification_time_different :=
			game_dll_modification_time != game_api.modification_time

		should_reload :=
			game_dll_modification_time_different && !is_there_error_getting_modification_time

		if should_reload {
			new_game_api, new_game_api_loaded_ok := load_game_api(game_api.api_version + 1)

			if new_game_api_loaded_ok {
				game_memory_struct_size_changed :=
